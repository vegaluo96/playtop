...
# 下行发送缓冲上限：48k·s16·mono ≈ 96KB/s。这是【纯 OOM 兜底】——只防「对端彻底不拉帧（断开但轨没停）」
# 时缓冲无限堆积，不该截断任何正常回复。带预合成时一条长回复可能领先播放十几秒（合成快于实时），
# 故上限给到 120s（≈11.5MB/通，仍有界）。早先设 12s 过紧，会把「长回复领先缓冲的尾部」丢掉 → 话说不完整。
MAX_TTS_BUF = RTC_RATE * 2 * 120   # ≈ 11.5 MB
...
if _OK:
    ...
    class _TTSTrack(MediaStreamTrack):
        ...
        def feed(self, pcm24: bytes) -> None:
            ...
            if len(self._buf) > MAX_TTS_BUF:   # 背压保护：超上限丢最旧，跳到最新语音，绝不无限堆积
                del self._buf[: len(self._buf) - MAX_TTS_BUF]
