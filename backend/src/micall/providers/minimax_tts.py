...
def _shared_client() -> "httpx.AsyncClient":
    ...
    音色试听走 asyncio.run（一次性循环），通话走 wsserver 常驻循环；同一 httpx client 跨循环复用会污染
    连接池 → PoolTimeout（实测：换几次音色试听后通话合成全卡）。每个循环各持一个 client。
    timeout：read 30s（流式首块可能慢，别误杀）、connect 5s、pool 10s（池满快速失败而非干等）；
