...
class ApiyiLLM(LLMProvider):
    def __init__(self, node: NodeConfig) -> None:
        ...
        # 瞬时错误重试（铁律2，走配置）：上游限流/网关抖/连接超时时，「首 token 出来前」退避重试，
        # 别让一通电话被一次瞬时抖动打死。已开吐则不重试（防重复输出）。默认重试 1 次、0.4s 起步退避。
        self._max_retries = max(0, int(node.params.get("max_retries", 1) or 0))
