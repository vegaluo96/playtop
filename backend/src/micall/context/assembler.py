...
def _emotion_instruction(emotion_map: dict[str, str]) -> str:
    ...
    return (
        ...
        "日常闲聊 → tender / neutral。"
        "【要真的笑、真的叹气，别只用文字】这些英文括号标签会被读成真实的人声，该用就用、别省："
        "真觉得好笑/逗趣时就放 (laughs) 笑出声 或 (chuckle) 轻笑；无奈/心疼/疲惫时放 (sighs) 叹口气；"
        "难过到哽咽时放 (sniffs)（MiniMax 没有「嚎啕大哭」的音，最多到抽鼻子/哽咽这一步）；"
        "惊讶/被逗到时 (gasps) 倒吸气；要喘口气时 (breath)。例：「这也太好笑了 (laughs)，你怎么想出来的。」"
        "「(sighs) 唉……今天是不是又被为难了。」放在该有反应的那一下，别每句都加、别堆。"
        "想要停顿/留白用 <#0.4#>（数字是秒），让节奏更像人，同样别滥用。"
