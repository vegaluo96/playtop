...
import { publicMarketOverview } from "../markets/overview";
...
export function seriesRows(series: SnapRow[], market: "ah" | "ou", tz: string) {
  ...
  return { rows, chart, startAt: series[0]?.captured_at ?? null };
}
...
export async function detailView(p: Panorama, tz: string, opts: { deep: boolean }) {
  ...
  const liveExt = (mk: "ah" | "ou" | "eu"): SnapRow[] => {
    ...
    if (latest && !isDisplayableLiveSnapshot(mk, latest)) return [];
    return rawRows
      .filter((r) => isDisplayableLiveSnapshot(mk, r))
      .map((r) => ({
  ...
  const merged = (pre: SnapRow[], mk: "ah" | "ou" | "eu") => {
    ...
    if (hasRawLive && liveRows.length === 0) return [];
