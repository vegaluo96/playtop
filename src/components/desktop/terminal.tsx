...
export function Terminal({ initialMatchId, initialTab, initialDrawer }: { initialMatchId?: number; initialTab?: DTab; initialDrawer?: boolean }) {
  ...
  return (
    <div className="desktop-root" style={{ position: "relative", width: "100%", height: "100%", display: "flex", flexDirection: "column", background: "var(--bg)", color: "var(--fg)", overflow: "hidden" }}>
      ...
      {/* ===== 顶栏 ===== */}
      <div style={{ flexShrink: 0, height: 52, display: "flex", alignItems: "center", gap: 18, padding: "0 20px", borderBottom: "1px solid var(--line)", background: "var(--card)" }}>
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4, whiteSpace: "nowrap", overflow: "hidden" }}>
                      ...
                      <span style={{ fontSize: 11, color: "var(--fg-3)", whiteSpace: "nowrap", minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", flexShrink: 1 }}>{leagueZh(m.leagueId, m.leagueName)}</span>
                      <span className="mono" style={{ fontSize: 11, color: "var(--fg-3)", whiteSpace: "nowrap", flexShrink: 0 }}>{hhmm(m.kickoff, prefs.tz)}</span>
                      {m.live && (
                        <span style={{ display: "flex", alignItems: "center", gap: 3, fontSize: 11, color: "var(--red)", fontWeight: 800, flexShrink: 0, whiteSpace: "nowrap" }}>
                          <span className="livepulse" style={{ width: 4, height: 4, borderRadius: "50%", background: "var(--red)" }} />
                          {m.ht ? "中场" : m.elapsed != null ? `${m.elapsed}'` : "LIVE"}
                        </span>
                      )}
                      {m.ex && (
                        <span className="mono" style={{ fontSize: 11, color: "var(--fg-3)", whiteSpace: "nowrap", flexShrink: 0 }}>
                  ...
                  <div style={{ flexShrink: 0, textAlign: "right", width: 62 }}>
                    <MarketValue v={m.masked ? "●●" : (m.ah?.text ?? "—")} pulse={m.masked ? null : m.ah?.chgAt} className="" small dim={m.masked} style={{ color: m.masked ? "var(--fg-3)" : "var(--gold)", justifyContent: "flex-end", fontWeight: 800 }} />
                    <MarketValue v={m.masked || !m.ah ? X : `${f2(m.ah.h)}/${f2(m.ah.a)}`} masked={m.masked} pulse={m.masked ? null : m.ah?.chgAt} dir={m.ah?.hd} small dim style={{ justifyContent: "flex-end" }} />
                  </div>
                  <div style={{ flexShrink: 0, textAlign: "right", width: 62 }}>
                    <MarketValue v={m.masked || !m.ou ? X : m.ou.text} masked={m.masked} pulse={m.masked ? null : m.ou?.chgAt} className="mono" small style={{ color: "var(--gold)", justifyContent: "flex-end", fontWeight: 800 }} />
                    <MarketValue v={m.masked || !m.ou ? X : `${f2(m.ou.h)}/${f2(m.ou.a)}`} masked={m.masked} pulse={m.masked ? null : m.ou?.chgAt} dir={m.ou?.hd} small dim style={{ justifyContent: "flex-end" }} />
                  </div>
                  <div style={{ flexShrink: 0, textAlign: "right", width: 50 }}>
                    {[m.eu?.h, m.eu?.d, m.eu?.a].map((vv, i) => (
                      <MarketValue key={i} v={m.masked || vv == null ? X : f2(vv)} masked={m.masked} pulse={m.masked ? null : m.eu?.chgAt} dir={i === 0 ? m.eu?.hd : i === 2 ? m.eu?.ad : 0} small dim={i === 1} style={{ justifyContent: "flex-end", color: i === 1 ? "var(--fg-3)" : "var(--fg-2)" }} />
                    ))}
                  </div>
        ...
        {!compactDesktop && <div style={{ display: "flex", flexDirection: "column", minHeight: 0, borderLeft: "1px solid var(--line)", background: "var(--bg)" }}>
          {radar.length > 0 && (
            <div style={{ flexShrink: 0, maxHeight: "30%", display: "flex", flexDirection: "column", borderBottom: "1px solid var(--line)" }}>
              <div style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 6, padding: "12px 14px 8px" }}>
                ...
                <span className="mono" style={{ fontSize: 11.5, color: "var(--red)", fontWeight: 800 }}>{radar.length} 场进行中</span>
              </div>
              <div style={{ flex: 1, overflowY: "auto", minHeight: 0, padding: "0 10px 8px" }}>
                {radar.map((r) => (
                  ...
                  >
                    ...
                    {r.ex?.cor && <span className="mono" style={{ flexShrink: 0, fontSize: 11, color: "var(--fg-3)" }}>角 {r.ex.cor}</span>}
                    <span style={{ flexShrink: 0, minWidth: 62, whiteSpace: "nowrap" }}>
          ...
          <div style={{ flex: 1.2, display: "flex", flexDirection: "column", minHeight: 0, borderBottom: "1px solid var(--line)" }}>
            <div style={{ flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px 8px" }}>
              <span style={{ fontSize: 13, fontWeight: 800 }}>指数异动</span>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
            <div style={{ flexShrink: 0, display: "flex", alignItems: "baseline", justifyContent: "space-between", padding: "12px 14px 8px" }}>
              <span style={{ fontSize: 13, fontWeight: 800 }}>AI 概率报告 · 本场</span>
              <span style={{ fontSize: 11.5, color: "var(--fg-3)" }}>唯一付费项</span>
            </div>
            <div style={{ flex: 1, overflowY: "auto", minHeight: 0, padding: "0 14px 14px" }}>
              <div onClick={() => setModal({ kind: "record" })} style={{ display: "flex", gap: 8, marginBottom: 10, cursor: "pointer" }}>
                {[
                  [record?.hitRate30 != null ? `${record.hitRate30}%` : "—", "近30天回测", "var(--gold)"],
                  [record ? `${record.yesterday.hit}/${record.yesterday.total}` : "—", "昨日回测", undefined],
                  [record?.streak ? `${record.streak} 连续命中` : "—", "回测状态", "var(--up)"],
                ].map(([v, label, color]) => (
                  <div key={label as string} style={{ flex: 1, background: "var(--card)", border: "1px solid var(--line)", borderRadius: 8, padding: "8px 0", textAlign: "center" }}>
                    <div className="mono" style={{ fontSize: 14, fontWeight: 800, color: color as string | undefined }}>{v as string}</div>
                ...
                <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 10, padding: "12px 14px" }}>
                  ...
                  {pred.locked ? (
                    <>
                      ...
                      <div style={{ textAlign: "center", fontSize: 11.5, color: "var(--fg-3)", marginTop: 6 }}>完整报告 · 可回看</div>
                    </>
                  ) : (
                    <>
              ...
              <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 10, padding: "10px 12px", marginTop: 10 }}>
                <div style={{ fontSize: 11, fontWeight: 800, marginBottom: 8 }}>近 7 日回测</div>
                    ...
                    return (
                      <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
                        ...
                        <div style={{ width: "100%", height: 26, display: "flex", alignItems: "flex-end" }}>
                          <div style={{ width: "100%", borderRadius: 2, background: pct == null ? "var(--line)" : pct >= 0.66 ? "var(--gold)" : "var(--fg-3)", height: pct == null ? 4 : Math.max(6, Math.round(pct * 26)) }} />
                        </div>
                ...
                <div style={{ borderTop: "1px solid var(--line-soft)", marginTop: 9, paddingTop: 8 }}>
                  <div style={{ fontSize: 11, fontWeight: 800, marginBottom: 4 }}>昨日报告复盘</div>
                  {(record?.yesterdayRows ?? []).length === 0 && <div style={{ fontSize: 11, color: "var(--fg-3)", padding: "4px 0" }}>昨日暂无已结算报告</div>}
                  {(record?.yesterdayRows ?? []).map((y: V, i: number) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 7, padding: "4px 0" }}>
                      <span style={{ flex: 1, fontSize: 11.5, color: "var(--fg-mid)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{y.match}</span>
                      <span style={{ fontSize: 11.5, fontWeight: 700, color: "var(--gold)", flexShrink: 0, whiteSpace: "nowrap" }}>{y.pick}</span>
                      <span className="mono" style={{ fontSize: 11.5, color: "var(--fg-2)", width: 26, textAlign: "right", flexShrink: 0 }}>{y.score}</span>
      ...
      <Modal open={modal?.kind === "unlock"} onClose={() => setModal(null)} width={420}>
        {modal?.kind === "unlock" && (
          <>
            <ModalTitle title={`解锁 AI 概率报告 · ${modal.data.match}`} />
            <div style={{ fontSize: 11, color: "var(--fg-2)", marginBottom: 14, lineHeight: 1.6 }}>概率摘要、指数解读与人员情报</div>
            <div style={{ display: "flex", background: "var(--inset)", borderRadius: 10, padding: "11px 14px", marginBottom: 14, justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 12, color: "var(--fg-2)" }}>
                价格 <span className="mono" style={{ fontSize: 16, fontWeight: 800, color: "var(--gold)" }}>{modal.data.price}</span> 额度
              </span>
              <span style={{ fontSize: 12, color: "var(--fg-2)" }}>
      ...
      <Modal open={modal?.kind === "recharge"} onClose={() => setModal(null)} width={520}>
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 4 }}>
          <span style={{ fontSize: 15, fontWeight: 800 }}>购买报告额度</span>
      ...
      <Modal open={modal?.kind === "record"} onClose={() => setModal(null)} width={480}>
        <ModalTitle title="历史回测" hint="概率摘要对照赛果自动统计" />
        ...
        <div style={{ borderTop: "1px solid var(--line)", paddingTop: 10 }}>
          ...
          {(record?.yesterdayRows ?? []).length === 0 && <div style={{ fontSize: 11, color: "var(--fg-3)" }}>昨日暂无已结算报告</div>}
          {(record?.yesterdayRows ?? []).map((y: V, i: number) => (
...
function LedgerModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Modal open={open} onClose={onClose} width={460}>
      <ModalTitle title="额度 / 解锁记录" hint={`共 ${rows.length} 笔`} />
      <div style={{ flex: 1, overflowY: "auto", minHeight: 0 }}>
...
function InviteLogModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Modal open={open} onClose={onClose} width={420}>
      <ModalTitle title="邀请记录" hint={`累计 ${data?.total ?? 0} 人 · +${data?.totalPts ?? 0} 额度`} />
      <div style={{ flex: 1, overflowY: "auto", minHeight: 0 }}>
      ...
      <div style={{ flexShrink: 0, fontSize: 11, color: "var(--fg-3)", marginTop: 8 }}>超出每日 10 / 每周 30 / 每月 100 上限的部分不计入</div>
    </Modal>
  );
}
