...
import { DesktopOddsSegmentedTabs, OddsCompareMatrix, OddsEuTrendPanel, OddsTrendPanel, type OddsMarketKey } from "@/components/odds-workbench";
...
export function CenterPane({
  ...
  const intelSide = (name: string, color: string, items: V[]) => (
    <Card style={{ padding: "10px 14px 6px" }}>
      ...
      {items.length === 0 && <div style={{ fontSize: 11.5, color: "var(--fg-3)", padding: "9px 0", borderTop: "1px solid var(--line-soft)" }}>暂无伤停通报</div>}
        ...
        <div className="hidescroll" style={{ display: "flex", gap: 4, borderBottom: "1px solid var(--line)", overflowX: "auto" }}>
          ...
          {tab === "odds" && (
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginLeft: 12 }}>
              <DesktopOddsSegmentedTabs value={oddsSub} onChange={setOddsSub} />
            </div>
          )}
        </div>
      </div>
        ...
        {tab === "odds" && oddsSub === "ah" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "minmax(420px,1.25fr) minmax(320px,0.75fr)", gap: 14, alignItems: "start" }}>
              <div>
                <div style={{ fontSize: 12, fontWeight: 850, margin: "0 2px 8px" }}>让球指数 · 初始 / 即时</div>
                <OddsCompareMatrix market="ah" rows={v.comp.ah} onHistory={(c) => c.bid && setHistory({ id: h.id, mk: "ah", bid: c.bid, co: c.co })} />
              </div>
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "minmax(420px,1.25fr) minmax(320px,0.75fr)", gap: 14, alignItems: "start" }}>
              <div>
                ...
                <OddsCompareMatrix market="eu" rows={v.comp.eu} euMeta={v.comp.euMeta} onHistory={(c) => c.bid && setHistory({ id: h.id, mk: "eu", bid: c.bid, co: c.co })} />
              </div>
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "minmax(420px,1.25fr) minmax(320px,0.75fr)", gap: 14, alignItems: "start" }}>
              <div>
                ...
                <OddsCompareMatrix market="ou" rows={v.comp.ou} onHistory={(c) => c.bid && setHistory({ id: h.id, mk: "ou", bid: c.bid, co: c.co })} />
              </div>
        ...
        {tab === "match" && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: 14 }}>
            ...
            <Card style={{ padding: "12px 14px" }}>
              ...
              {h.live && (
                <div style={{ borderTop: "1px solid var(--line-soft)", marginTop: 12, paddingTop: 10 }}>
                  ...
                  {!v.tech.stats && <div style={{ fontSize: 11, color: "var(--fg-3)", padding: "6px 0" }}>暂无技术统计,开赛后更新</div>}
            ...
            <Card style={{ padding: "12px 14px" }}>
              ...
              {!v.tech.minutes && <div style={{ fontSize: 11.5, color: "var(--fg-3)" }}>暂无进球时段数据</div>}
            ...
            <Card style={{ padding: "12px 14px", alignSelf: "start" }}>
              ...
              {(v.tech.standings?.table ?? []).length > 0 && (
                <div style={{ borderTop: "1px solid var(--line-soft)", marginTop: 10, paddingTop: 8 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 6 }}>积分榜 <span style={{ fontSize: 11, color: "var(--fg-3)", fontWeight: 400 }}>两队高亮</span></div>
        ...
        {tab === "squad" &&
          (v.lineups.ready ? (
            ...
            <div style={{ background: "var(--card)", border: "1px dashed var(--line)", borderRadius: 12, padding: "44px 20px", textAlign: "center", maxWidth: 560, margin: "20px auto 0" }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "var(--fg-2)" }}>首发尚未公布</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(340px,1fr))", gap: 14 }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <Card style={{ padding: "8px 14px 4px" }}>
                  ...
                  {(deepV.scorers ?? []).length === 0 && <div style={{ fontSize: 11.5, color: "var(--fg-3)", padding: "7px 0" }}>榜单暂无数据</div>}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {deepV.seasonPanel && (deepV.seasonPanel.home || deepV.seasonPanel.away) && (
                  <Card style={{ padding: "8px 14px" }}>
                    <div style={{ fontSize: 12, fontWeight: 700, padding: "4px 0 6px" }}>赛季面板 <span style={{ fontSize: 11, color: "var(--fg-3)", fontWeight: 400 }}>主客拆分</span></div>
                ...
                <Card style={{ padding: "8px 14px 4px" }}>
                  ...
                  {(deepV.lb ?? []).length === 0 && <div style={{ fontSize: 10.5, color: "var(--fg-3)", padding: "7px 0" }}>榜单数据积累中</div>}
                  {deepV.lb?.map((b: V) => (
                ...
                <Card style={{ padding: "8px 14px 4px" }}>
                  ...
                  {(deepV.ratings ?? []).length === 0 && <div style={{ fontSize: 11.5, color: "var(--fg-3)", padding: "7px 0" }}>评分暂无数据</div>}
                ...
                <Card style={{ padding: "10px 14px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {(deepV.motiv ?? []).length === 0 && <span style={{ fontSize: 11.5, color: "var(--fg-3)" }}>暂无荣誉数据</span>}
