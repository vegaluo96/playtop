...
export function RefreshRules({ activeIdx, intervals }: { activeIdx: number | null; intervals: number[] | null }) {
  ...
  const groupTitle = (t: string) => (
    <div style={{ fontSize: 11, fontWeight: 800, color: "var(--fg-3)", letterSpacing: 1, padding: "9px 12px 3px" }}>{t}</div>
  );
  ...
  return (
    <>
      ...
      <div style={{ position: "relative" }}>
        {/* 阶梯连线:节点由远及近渐亮 */}
        <span style={{ position: "absolute", left: 18, top: 14, bottom: 14, width: 1, background: "var(--line)" }} />
  ...
  return (
    <Sheet open onClose={onClose}>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 6 }}>
        <span style={{ fontSize: 16, fontWeight: 800 }}>数据刷新规则</span>
        <span style={{ fontSize: 11.5, color: "var(--fg-3)" }}>越临近开球,刷新越快</span>
      </div>
