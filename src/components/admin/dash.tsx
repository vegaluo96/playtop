    ...
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,minmax(0,1fr))", gap: 12, marginBottom: 14 }}>
        {v.kpis.map((k: V) => (
          <div key={k.label} style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 12, padding: "13px 14px" }}>
            <div style={{ fontSize: 11, color: "var(--fg-2)", marginBottom: 6 }}>{k.label}</div>
            <div className="mono" style={{ fontSize: 20, fontWeight: 800, color: kpiColor(k.c), whiteSpace: "nowrap" }}>{k.v}</div>
            <div style={{ fontSize: 11, marginTop: 4, color: k.delta?.startsWith("↑") ? "var(--green)" : k.delta?.startsWith("↓") ? "var(--red)" : "var(--fg-2)", minHeight: 12 }}>{k.delta}</div>
          </div>
        ))}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1.2fr 1fr 1fr", gap: 14 }}>
        ...
        <ACard title="近 7 日新增注册">
          <div style={{ display: "flex", gap: 7, alignItems: "flex-end", height: 128 }}>
            {v.week.map((b: V) => (
              <div key={b.d} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4, height: "100%", justifyContent: "flex-end" }}>
                <span className="mono" style={{ fontSize: 11, color: "var(--fg-2)" }}>{b.reg > 0 ? b.reg : ""}</span>
                <div style={{ width: "100%", borderRadius: "3px 3px 0 0", background: "var(--home)", height: Math.max(2, Math.round((b.reg / maxReg) * 88)) }} />
                <span className="mono" style={{ fontSize: 11, color: "var(--fg-3)" }}>{b.d}</span>
              </div>
            ))}
          </div>
        </ACard>
        <ACard title="API 配额" right={<span className="mono" style={{ fontSize: 11.5, color: "var(--fg-3)" }}>{v.af?.plan ?? "—"} · /status</span>}>
          ...
          <div style={{ borderTop: "1px solid var(--line-soft)", marginTop: 9, paddingTop: 8, display: "flex", justifyContent: "space-between" }}>
            <span style={{ fontSize: 11, color: "var(--fg-2)" }}>快照归档(今日)</span>
            <span style={{ fontSize: 11, fontWeight: 700, color: "var(--green)" }}>{v.snapsToday.toLocaleString()} 条</span>
          </div>
        </ACard>
        <ACard title="大模型配额" right={<span className="mono" style={{ fontSize: 11.5, color: "var(--fg-3)" }}>AI 报告生成</span>}>
          <div className="mono" style={{ fontSize: 18, fontWeight: 800 }}>
            {(v.llm.tokens / 1e6).toFixed(2)}M <span style={{ fontSize: 11, color: "var(--fg-3)", fontWeight: 400 }}>/ {(v.llm.budget / 1e6).toFixed(0)}M tokens</span>
          </div>
          <div style={{ height: 7, background: "var(--inset)", borderRadius: 4, overflow: "hidden", margin: "8px 0" }}>
          ...
          <div style={{ borderTop: "1px solid var(--line-soft)", marginTop: 9, paddingTop: 8, display: "flex", justifyContent: "space-between" }}>
            <span style={{ fontSize: 11, color: "var(--fg-2)" }}>网关余额</span>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1.2fr 1fr 1fr", gap: 14, marginTop: 14 }}>
        <ACard title="今日热门场次 Top 5" pad={false}>
          ...
          {v.hot.length === 0 && <div style={{ padding: 14, fontSize: 11.5, color: "var(--fg-3)" }}>今日暂无浏览数据(埋点随访问累积)</div>}
          {v.hot.map((h: V) => (
            <AGrid key={h.m} cols="1.4fr 70px 56px 64px">
              ...
              <span className="mono" style={{ fontSize: 11.5, textAlign: "right", color: "var(--fg-2)" }}>{h.pv.toLocaleString()}</span>
              <span className="mono" style={{ fontSize: 11.5, fontWeight: 700, textAlign: "right", color: "var(--gold)" }}>{h.free ? "—" : h.un}</span>
              <span style={{ fontSize: 11, fontWeight: 700, textAlign: "right", color: h.free ? "var(--green)" : "var(--gold)" }}>{h.rate}</span>
            </AGrid>
          ))}
        </ACard>
        <ACard title="今日收入构成(购买额度)">
          {v.revmix.length === 0 && <div style={{ fontSize: 11.5, color: "var(--fg-3)" }}>今日暂无购买</div>}
          {v.revmix.map((r: V) => (
            <div key={r.k} style={{ marginBottom: 9 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                <span className="mono" style={{ fontSize: 11.5, color: "var(--fg-2)" }}>{r.k}</span>
        ...
        <ACard title="额度流水(今日)">
          <div style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: "1px solid var(--line-soft)" }}>
            <span style={{ fontSize: 11.5, color: "var(--fg-2)" }}>发放</span>
            <span className="mono" style={{ fontSize: 11, fontWeight: 700, color: "var(--green)" }}>+{v.flow.grant.toLocaleString()}</span>
          </div>
          <div style={{ fontSize: 11, color: "var(--fg-3)", padding: "4px 0 7px", borderBottom: "1px solid var(--line-soft)" }}>
            {v.flow.grantMix.map((g: V) => `${({ recharge: "购买", gift: "礼包", invite: "邀请", redeem: "兑换", adjust: "调整" } as V)[g.kind] ?? g.kind} ${g.v.toLocaleString()}`).join(" · ") || "—"}
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: "1px solid var(--line-soft)" }}>
            <span style={{ fontSize: 11.5, color: "var(--fg-2)" }}>消耗(解锁报告)</span>
            <span className="mono" style={{ fontSize: 11, fontWeight: 700, color: "var(--red)" }}>-{v.flow.consume.toLocaleString()}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "7px 0" }}>
            <span style={{ fontSize: 11.5, color: "var(--fg-2)" }}>净增负债</span>
            <span className="mono" style={{ fontSize: 11, fontWeight: 800, color: "var(--gold)" }}>{v.flow.net >= 0 ? "+" : ""}{v.flow.net.toLocaleString()}</span>
          </div>
          <div style={{ fontSize: 11, color: "var(--fg-3)", marginTop: 4 }}>负债总额 {v.flow.debt.toLocaleString()} ≈ ¥{Math.round(v.flow.debt / 10).toLocaleString()}</div>
        </ACard>
        <ACard title="今日转化漏斗">
            ...
            return (
              <div key={f.label} style={{ marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                  <span style={{ fontSize: 11.5, color: "var(--fg-2)" }}>{f.label}</span>
                  <span className="mono" style={{ fontSize: 11.5, fontWeight: 700 }}>{f.v.toLocaleString()} <span style={{ color: "var(--fg-3)" }}>{f.pct}</span></span>
                </div>
