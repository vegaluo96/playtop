...
const TYPES = ["数据问题", "购买额度问题", "功能建议", "其他"];
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "6px 12px 16px", minHeight: 0 }}>
        <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, padding: 14 }}>
          ...
          <textarea
            id="tk-desc"
            rows={4}
            placeholder="请描述你遇到的问题或建议,如:某场比赛指数数据延迟…"
          ...
          />
          {msg && <div style={{ fontSize: 11, color: msg.ok ? "var(--up)" : "var(--red)", marginTop: 6, textAlign: msg.ok ? "center" : "left" }}>{msg.text}</div>}
              ...
              <div key={t.id} style={{ padding: "11px 0", borderBottom: "1px solid var(--line-soft)" }}>
                ...
                {t.reply && (
                  <div style={{ marginTop: 6, background: "var(--inset)", borderRadius: 8, padding: "8px 10px", fontSize: 11, color: "var(--fg-mid)", lineHeight: 1.6 }}>
                    <span style={{ fontSize: 11, fontWeight: 800, color: "var(--green)", marginRight: 6 }}>客服回复</span>
                    {t.reply}
                  </div>
                )}
              </div>
