...
1. 后台 → 设置：填 apiyi Key（建议选带联网检索的模型）→「测试连接」；点「导入国际赛历史」+「Elo 全量回放」。
2. 后台 → 比赛管理 →「手动建赛」：联赛代码 `WC2026`、国家/地区填 `国际`、队名用历史库英文名（如 `Argentina`/`France`，与 martj42 一致）、勾选中立场（美加墨东道主场次不勾）、场馆填球场名（自动地理编码喂天气）。
3. 工作台：手动录入盘口（从任意盘口网站抄 1X2/大小/亚盘）→「采集（含 AI）」→「运行引擎 + 生成报告」→ 审阅草稿（可改定性段落）→ 定价 → 发布。
